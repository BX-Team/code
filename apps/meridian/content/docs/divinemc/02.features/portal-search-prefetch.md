...
::callout{type="info"}
  This feature pairs well with `misc.lag-compensation.portal-acceleration`: acceleration shortens the wait inside the portal on a lagging server, while the prefetch makes sure the teleport itself does not spike the tick.
::
