...
| GET | `/auth/me` | сессия | `{user}` |
| GET | `/auth/session` | — | `{user}` или `null` — спрашивается на каждой навигации |
...
Секрет подписи сессий не нужен: токен сессии — 32 случайных байта, в базе лежит только его
SHA-256, а кука не несёт ничего кроме самого токена. Подписывать нечего, зато отзыв сессии —
это `DELETE` строки, а не ожидание истечения подписи.
